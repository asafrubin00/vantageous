...
function TermsView({ onBack }) {
  return (
    <div className="max-w-2xl mx-auto py-8 sm:py-12">
      <button onClick={onBack} className="text-gray-500 hover:text-salmon-dim text-sm transition-colors mb-8">
        ← Back to Vantageous
      </button>

      <h1 className="font-headline text-salmon text-3xl mb-8">Terms of Use</h1>

      <div className="space-y-5 text-gray-300 text-sm leading-relaxed">
        <p>
          Vantageous and all associated content — including the signal analysis, valuation
          models, governance of the underlying data, and written copy — are the proprietary
          work of{' '}
          <a
            href="https://asafrubin00.github.io/asaf-rubin-website/"
            target="_blank"
            rel="noopener noreferrer"
            className="text-salmon-dim underline hover:text-salmon transition-colors"
          >
            Asaf Rubin
          </a>{' '}
        ...
        <p>
          This site is made available for personal, non-commercial use only. No part of
          Vantageous may be reproduced, distributed, licensed, or used for commercial purposes
          without the express prior written permission of the author.
        </p>

        <p className="text-gray-400">
          <strong className="text-gray-200">Nothing here is investment advice.</strong> Signals
          are generated by an AI model reading public news and may be wrong, incomplete, or out
          of date. Valuations are arithmetic applied to public filings under assumptions you
          choose yourself; changing those assumptions changes the answer substantially, and the
          output is a model result rather than a recommendation. Nothing on this site should be
          relied upon when making financial decisions.
        </p>

        <p className="text-gray-400">
          Company financials come from SEC EDGAR, prices from public market data, and news from
          the publishers listed in the footer. None of those sources are affiliated with or
          endorse this site, and their data is used as published without any warranty as to its
          accuracy.
        </p>

        <p>
          For licensing or partnership enquiries, contact:{' '}
          <a href="mailto:rubin.asaf01@gmail.com" className="text-salmon-dim underline hover:text-salmon transition-colors">
...
export default function App() {
  // Terms lives on a hash so the link is shareable and survives a reload, without
  // pulling a router into a three-view app.
  const [mode, setMode] = useState(() =>
    typeof window !== 'undefined' && window.location.hash === '#terms' ? 'terms' : 'signals'
  );
  ...
  // Follow the hash both ways, so the browser back button leaves Terms as expected.
  useEffect(() => {
    const sync = () => setMode((m) => (window.location.hash === '#terms' ? 'terms' : m === 'terms' ? 'signals' : m));
    window.addEventListener('hashchange', sync);
    return () => window.removeEventListener('hashchange', sync);
  }, []);

  const leaveTerms = useCallback(() => {
    ...
    <RatingsProvider>
      ...
      <div className="min-h-screen bg-dark font-body">
          ...
          <div className="max-w-6xl mx-auto px-3 sm:px-4 pt-4 pb-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center bg-dark-card border border-dark-border rounded-lg p-0.5 gap-0.5 shrink-0">
                ...
                >Signals</button>
                <button onClick={() => { leaveTerms(); setMode('briefing'); }}
                ...
                >Briefing</button>
                <button onClick={() => { leaveTerms(); setMode('valuation'); }}
        ...
        <main className="max-w-6xl mx-auto px-3 sm:px-4 py-5">
          ...
          {mode === 'terms' && <TermsView onBack={leaveTerms} />}
          ...
          <div className={mode === 'valuation' ? '' : 'hidden'} aria-hidden={mode !== 'valuation'}>
            <ValuationView ticker={valuationTicker} onTicker={setValuationTicker} />
          </div>
        </main>

        {/* mt-12 keeps clear of the content above, and the footer sits in normal flow
            rather than fixed, so it can never overlap a view that scrolls. */}
        <footer className="border-t border-dark-border mt-12 py-5 px-3 text-center text-gray-700 text-[11px] space-y-1.5">
          <p>FT · BBC · Guardian · CNBC · Economist · Yahoo · MarketWatch &amp; more · Analysis by Claude · Not financial advice</p>
          <p>
            © 2026{' '}
            <a
              href="https://asafrubin00.github.io/asaf-rubin-website/"
              target="_blank"
              rel="noopener noreferrer"
              className="underline hover:text-salmon-dim transition-colors"
            >
              Asaf Rubin
            </a>
            . All rights reserved.
            <span className="mx-1.5 text-gray-800">·</span>
            <a href="#terms" className="underline hover:text-salmon-dim transition-colors">Terms of Use</a>
          </p>
        </footer>
